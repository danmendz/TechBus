<div class="w-full mx-auto p-6 bg-white shadow-md rounded-lg">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="space-y-4">
            <!-- Dropdown para seleccionar la corrida -->
            <div>
                <label for="selectedCorridaId" class="block text-sm font-medium text-gray-700">
                    Seleccionar Corrida
                </label>
                <select wire:model="selectedCorridaId" id="selectedCorridaId" 
                    wire:change="loadUsersByCorrida"
                    ...
                    @foreach ($corridas as $corrida)
                        <option value="{{ $corrida->id }}">
                            {{ $corrida->fecha }} - {{ $corrida->estatus_corrida }}
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="grid grid-cols-1 gap-4">
                ...
                <div>
                    ...
                    <input wire:model="notificationForm.descripcion" type="text" id="descripcion" 
                        class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm 
                        focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm">
                </div>
        ...
        <div class="space-y-6">
            ...
            <div class="p-4 bg-gray-50 rounded-lg shadow-md">
                ...
                @else
                    <p class="text-gray-500">Seleccione una corrida para ver los usuarios</p>
                @endif
            </div>
            ...
            <div class="p-4 bg-gray-50 rounded-lg shadow-md">
                ...
                @else
                    <p class="text-gray-500">No hay usuarios con errores</p>
                @endif
            </div>
        </div>
    </div>
